...
/// Scan the data directory and sync all files to the database
pub async fn sync_filesystem_to_db(pool: &SqlitePool, default_owner_id: &str) -> anyhow::Result<usize> {
    ...
    synced_count += scan_directory(pool, data_path, "", default_owner_id).await?;
    
    println!("✅ Filesystem sync complete: {} files synced", synced_count);
    Ok(synced_count)
}

/// Recursively scan a directory and sync files
fn scan_directory<'a>(
    pool: &'a SqlitePool,
    dir_path: &'a Path,
    relative_path: &'a str,
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        // Skip database files and search index
        ...
        if metadata.is_dir() {
            ...
            count += scan_directory(pool, &path, &file_relative_path, owner_id).await?;
        } else {
