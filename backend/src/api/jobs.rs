...
async fn list_jobs_handler(
    ...
    let jobs: Vec<JobRow> = sqlx::query_as(&sql)
...
async fn get_job_handler(
    ...
    let job: JobRow = sqlx::query_as("SELECT * FROM jobs WHERE id = ?")
        .bind(&job_id)
        .fetch_one(&state.db_pool)
        .await
...
async fn cancel_job_handler(
    ...
    let result = sqlx::query("UPDATE jobs SET status = 'cancelled', completed_at = ? WHERE id = ? AND status IN ('pending', 'running')")
    ...
    Ok(Json(
        serde_json::json!({"success": true, "message": "Job cancelled"}),
    ))
}
...
async fn cleanup_jobs_handler(
    ...
    let result = sqlx::query("DELETE FROM jobs WHERE status IN ('completed', 'failed', 'cancelled') AND completed_at < ?")
