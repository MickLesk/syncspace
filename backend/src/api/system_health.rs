...
async fn get_system_health(
    ...
    // Get uptime - calculate from start_time (Unix timestamp) to now
    let now = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .unwrap()
        .as_secs();
    let uptime_seconds = now.saturating_sub(state.start_time);
...
fn get_server_metrics() -> ServerMetrics {
    ...
    // Get system info - use environment variable or fallback
