...
use axum::{extract::{Path, State, Query}, http::StatusCode, routing::{get, put, post}, Json, Router};
...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/shares/{share_id}/regenerate-token", post(regenerate_share_token))
...
async fn get_access_log(
    ...
    // WHERE share_id = ? ORDER BY timestamp DESC
...
async fn get_public_share(
    ...
    let share: crate::database::SharedLink = sqlx::query_as(
        "SELECT * FROM shared_links WHERE id = ? AND is_public = 1"
    )
    .bind(&share_token)
...
async fn download_public_share(
    ...
    // Get share by token
    let mut share: crate::database::SharedLink = sqlx::query_as(
    ...
    Ok((StatusCode::NOT_IMPLEMENTED, "File streaming not yet implemented"))
}
