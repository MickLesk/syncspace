...
>
  <tr>
    {#each columns as column}
      <th
        class="px-4 py-3 {getAlignmentClass(column.align)} {compact
          ? 'text-xs'
          : 'text-sm'} font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider"
      >
        {#if column.sortable !== false}
          <button
            class="flex items-center gap-2 hover:text-primary-600 dark:hover:text-primary-400 transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 rounded px-1 {column.align ===
            'center'
          ...
          >
            ...
            {#if sortBy === column.key}
              <i
                class="bi bi-{sortOrder === 'asc'
                  ? 'arrow-up'
                  : 'arrow-down'} text-primary-600 dark:text-primary-400"
                aria-hidden="true"
              ></i>
            {:else}
              <i
