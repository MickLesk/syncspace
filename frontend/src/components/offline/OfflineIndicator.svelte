<script>
  ...
  const { isOnline, isSyncing, offlineQueue, isOfflineMode } = offlineManager;
...
{#if $isOfflineMode}
  <div
    class="sticky top-16 md:top-0 z-20 bg-amber-50 dark:bg-amber-900 border-b-2 border-amber-300 dark:border-amber-700 px-4 py-3 shadow-sm"
  >
      ...
      <div class="flex items-center gap-2 flex-1 min-w-0">
        {#if !$isOnline}
          <div class="flex items-center gap-2">
            <i
              class="bi bi-wifi-off text-amber-700 dark:text-amber-200 text-lg flex-shrink-0"
            ></i>
            <span
              class="text-sm font-semibold text-amber-900 dark:text-amber-100 truncate"
            >
        ...
        {:else if $offlineQueue.length > 0}
          <div class="flex items-center gap-2">
            <i
              class="bi bi-exclamation-circle text-orange-600 dark:text-orange-400 text-lg flex-shrink-0"
            ></i>
            <span
              class="text-sm font-semibold text-orange-900 dark:text-orange-100 truncate"
            >
              {$offlineQueue.length +
                " pending change" +
    ...
    {#if showDetails && $offlineQueue.length > 0}
      <div class="mt-3 pt-3 border-t border-amber-200 dark:border-amber-700">
        <p
          class="text-xs text-amber-900 dark:text-amber-200 mb-2 font-semibold"
        >
          Pending changes:
        </p>
        <div class="space-y-1 max-h-32 overflow-y-auto">
          {#each $offlineQueue as operation, i}
            <div
              class="text-xs text-amber-800 dark:text-amber-300 flex items-center gap-2 px-2 py-1 bg-white dark:bg-gray-800 rounded"
            >
              <i
                class={`bi bi-${getOperationIcon(operation.method)} flex-shrink-0`}
              ></i>
              <span class="truncate flex-1">{operation.url}</span>
              <span
                class="text-amber-600 dark:text-amber-400 text-xs flex-shrink-0"
              >
                #{i + 1}
              </span>
            </div>
