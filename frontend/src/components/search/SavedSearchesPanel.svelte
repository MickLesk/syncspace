<script>
  ...
  function handleDuplicateSearch(search) {
    const newName = prompt(t('savedSearches.duplicateName'), `${search.name} (Copy)`);
    if (newName) {
...
<div class="border-b border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800">
  ...
  {#if showPanel}
    <div class="border-t border-gray-200 p-4 dark:border-gray-700 space-y-4">
      ...
      <div class="space-y-2">
        ...
        <div class="flex gap-2">
          ...
          <button
            ...
            class="px-3 py-2 rounded bg-blue-500 text-white text-sm font-medium hover:bg-blue-600 disabled:opacity-50 transition-colors"
          >
            {t('common.save')}
          </button>
        </div>
            ...
            >
                ...
                {#if search.tags?.length > 0}
                  <div class="flex gap-1 mt-1 flex-wrap">
                    {#each search.tags as tag}
                      <span class="inline-block px-1.5 py-0.5 rounded bg-gray-200 dark:bg-gray-600 text-xs text-gray-700 dark:text-gray-300">
                        {tag}
                      </span>
                    {/each}
                  </div>
              ...
              <div class="flex gap-1">
                <button
                  ...
                  title={search.isFavorite ? t('common.unfavorite') : t('common.favorite')}
                ...
                >
                  <i class={`bi ${search.isFavorite ? 'bi-star-fill' : 'bi-star'}`}></i>
                </button>
      ...
      <!-- Import/Export & Clear -->
      <div class="border-t border-gray-200 pt-3 dark:border-gray-700 flex gap-2 flex-wrap">
        <button
      ...
      {#if showImportExport}
        <div class="border-t border-gray-200 pt-3 space-y-2 dark:border-gray-700">
          <button
