<script>
  ...
  async function loadFileTypes() {
    ...
    fileTypes = [
      ...
      {
        id: "spreadsheet",
        label: t("fileTypes.spreadsheet"),
        icon: "bi-file-earmark-spreadsheet",
      },
      {
        id: "archive",
        label: t("fileTypes.archive"),
...
>
  ...
  <div class="flex items-center justify-between mb-3">
    ...
    >
      ...
      {#if $activeCount > 0}
        <span
          class="ml-2 inline-block min-w-5 px-2 py-0.5 rounded-full bg-green-500 text-white text-xs font-semibold"
        >
          {$activeCount}
        </span>
      {/if}
    ...
    >
      ...
      <div>
        ...
        <div class="flex flex-wrap gap-2">
          {#each fileTypes as type}
            <button
              onclick={() => {
                $advancedFilters.updateFilter(
                  "fileType",
                  $advancedFilters.filters.fileType === type.id ? null : type.id
                );
                handleFilterChange();
              }}
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-2">
          <input
            ...
            onchange={(e) => {
              $advancedFilters.updateNestedFilter(
                "sizeRange.min",
                e.target.value ? parseInt(e.target.value) * 1024 * 1024 : null
              );
              handleFilterChange();
            }}
      ...
      <div>
        ...
        <div class="grid grid-cols-2 gap-2">
          <input
            type="date"
            value={$advancedFilters.filters.dateRange.from
              ? new Date($advancedFilters.filters.dateRange.from)
                  .toISOString()
                  .split("T")[0]
              : ""}
            onchange={(e) => {
          ...
          <input
            type="date"
            value={$advancedFilters.filters.dateRange.to
              ? new Date($advancedFilters.filters.dateRange.to)
                  .toISOString()
                  .split("T")[0]
              : ""}
            onchange={(e) => {
      ...
      <div class="border-t border-gray-200 pt-4 dark:border-gray-700">
        ...
        <div class="flex gap-2 mb-2">
          ...
          <button
            ...
            class="px-3 py-2 rounded bg-green-500 text-white text-sm font-medium hover:bg-green-600 disabled:opacity-50"
          >
            {t("filters.save")}
          </button>
        </div>
