    ...
    {:else}
      ...
      {#if showCreateForm && !readOnly}
        <div class="bg-blue-50 dark:bg-blue-900/20 p-4 rounded mb-4 space-y-3">
          <h3 class="font-medium text-blue-900 dark:text-blue-100">Create New Share</h3>

          <!-- Permission -->
          <div>
            <label class="text-sm font-medium text-slate-700 dark:text-slate-300 block mb-1">
              Permission
            </label>
            <select
      ...
      {:else}
        <div class="space-y-3">
          {#each $activeShares as share (share.id)}
            <div class="border border-slate-200 dark:border-slate-700 rounded p-3 space-y-2">
              <!-- Header -->
              <div class="flex items-start justify-between gap-2">
                <div>
                  <p class="text-sm font-medium text-slate-900 dark:text-white">
                    ...
                    {#if share.password}
                      <i class="bi bi-lock-fill text-amber-500 ml-1" title="Password protected" />
                    {/if}
                  </p>
                  <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">
                    Created {new Date(share.createdAt).toLocaleDateString('de-DE')}
                  </p>
                </div>

                <div
                  class="px-2 py-1 rounded text-xs font-medium"
                  class:bg-green-100={!isShareExpired(share.expiresAt)}
                  class:dark:bg-green-900/30={!isShareExpired(share.expiresAt)}
                  class:text-green-700={!isShareExpired(share.expiresAt)}
                  class:dark:text-green-200={!isShareExpired(share.expiresAt)}
                  class:bg-red-100={isShareExpired(share.expiresAt)}
                  class:dark:bg-red-900/30={isShareExpired(share.expiresAt)}
                  class:text-red-700={isShareExpired(share.expiresAt)}
                  class:dark:text-red-200={isShareExpired(share.expiresAt)}
                >
              ...
              <!-- Share Link -->
              <div class="bg-slate-50 dark:bg-slate-700/50 p-2 rounded text-xs font-mono text-slate-600 dark:text-slate-400 break-all">
                {share.shareToken}
              </div>
