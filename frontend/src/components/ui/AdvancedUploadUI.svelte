<script>
  ...
  import { uploadManager, formatBytes, formatTime } from '../../stores/uploadManager.js';
...
{#if stats.total > 0}
  <div class="fixed bottom-0 right-0 m-4 w-96 bg-white dark:bg-gray-800 rounded-lg shadow-2xl overflow-hidden z-40">
    <!-- Header -->
    <div
    ...
    >
      ...
      <button onclick={(e) => { e.stopPropagation(); isExpanded = !isExpanded; }} class="text-xl">
        {isExpanded ? '▼' : '▲'}
      </button>
    </div>
    ...
    {#if isExpanded}
      ...
      <div class="p-4 border-b border-gray-200 dark:border-gray-700">
        <div class="flex justify-between mb-2 text-sm">
          ...
          <span class="font-semibold text-gray-900 dark:text-white">{stats.percentage}%</span>
        </div>
        ...
        {#each jobs as job (job.id)}
          <div class="p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
            <div class="flex items-center gap-3 mb-2">
              <i class={`bi ${getStatusIcon(job.state)} ${getStatusColor(job.state)}`}></i>
              <div class="flex-1 min-w-0">
                <p class="font-medium text-gray-900 dark:text-white truncate">{job.filename}</p>
                <p class="text-xs text-gray-500 dark:text-gray-400 truncate">{job.destination}</p>
              </div>
              ...
              <div class="mb-2">
                ...
                <div class="flex justify-between text-xs text-gray-600 dark:text-gray-400">
                  ...
                  {#if job.state === 'uploading'}
                    <span>
                      {formatBytes(job.speedBytesPerSecond)}/s · ETA: {formatTime(job.estimatedTimeRemaining)}
                    </span>
                  {/if}
                </div>
              </div>
            ...
            {#if job.error}
              <p class="text-xs text-red-600 dark:text-red-400 mb-2">{job.error}</p>
            {/if}
