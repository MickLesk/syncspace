  ...
  <div class="card-body p-4">
    ...
    <div class="mb-3 flex items-start justify-between">
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2">
          <i class="bi {getStatusIcon(job.status)} text-lg" class:text-success={job.status === 'completed'} class:text-error={job.status === 'failed'} class:text-warning={job.status === 'pending'} class:text-info={job.status === 'processing'}></i>
          <span class="badge {getStatusClass(job.status)} badge-sm">
            {t($language, `conversion.status${job.status.charAt(0).toUpperCase() + job.status.slice(1)}`)}
          </span>
        </div>
        <h3 class="mt-2 truncate text-sm font-semibold text-base-content" title={job.source_path}>
    ...
    {#if job.status === 'processing' && job.progress !== null && job.progress !== undefined}
      <div class="mb-3">
        ...
        <progress class="progress progress-primary w-full" value={job.progress} max="100"></progress>
      </div>
    {/if}
