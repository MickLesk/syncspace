  ...
  {#if !$activityFeed.isLoading}
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
      ...
      <div class="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/20 dark:to-blue-900/10 rounded-lg p-4 border border-blue-200 dark:border-blue-700">
        <p class="text-sm font-medium text-blue-700 dark:text-blue-300 mb-1">{t('activity.total')}</p>
        <p class="text-3xl font-bold text-blue-900 dark:text-blue-100">{$activityFeed.stats.totalActivities}</p>
        <p class="text-xs text-blue-600 dark:text-blue-400 mt-1">{t('activity.all_time')}</p>
      </div>
      ...
      <div class="bg-gradient-to-br from-green-50 to-green-100 dark:from-green-900/20 dark:to-green-900/10 rounded-lg p-4 border border-green-200 dark:border-green-700">
        <p class="text-sm font-medium text-green-700 dark:text-green-300 mb-1">{t('activity.today')}</p>
        <p class="text-3xl font-bold text-green-900 dark:text-green-100">{$activityFeed.stats.today}</p>
        <p class="text-xs text-green-600 dark:text-green-400 mt-1">{t('activity.events_today')}</p>
      </div>

      <!-- This Week -->
      <div class="bg-gradient-to-br from-yellow-50 to-yellow-100 dark:from-yellow-900/20 dark:to-yellow-900/10 rounded-lg p-4 border border-yellow-200 dark:border-yellow-700">
        <p class="text-sm font-medium text-yellow-700 dark:text-yellow-300 mb-1">{t('activity.this_week')}</p>
        <p class="text-3xl font-bold text-yellow-900 dark:text-yellow-100">{$activityFeed.stats.thisWeek}</p>
        <p class="text-xs text-yellow-600 dark:text-yellow-400 mt-1">{t('activity.last_7_days')}</p>
      </div>
      ...
      <div class="bg-gradient-to-br from-purple-50 to-purple-100 dark:from-purple-900/20 dark:to-purple-900/10 rounded-lg p-4 border border-purple-200 dark:border-purple-700">
        <p class="text-sm font-medium text-purple-700 dark:text-purple-300 mb-1">{t('activity.this_month')}</p>
        <p class="text-3xl font-bold text-purple-900 dark:text-purple-100">{$activityFeed.stats.thisMonth}</p>
        <p class="text-xs text-purple-600 dark:text-purple-400 mt-1">{t('activity.last_30_days')}</p>
      </div>
    </div>
  {/if}
    ...
    <div class="mb-4">
      <div class="relative">
        ...
        <input
          ...
          class="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg dark:bg-gray-800 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
        />
      </div>
    </div>
    ...
    <div class="flex flex-wrap gap-2 mt-4">
      {#each ['all', ...Object.keys(ACTIVITY_TYPES)] as type}
        <button
          ...
          class:bg-blue-600={selectedType === type}
      ...
      {#each $activityFeed.filtered as activity (activity.id)}
        ...
        >
          <div class="flex items-start gap-4">
            ...
            >
              <i class="{getActivityIcon(activity.type)} text-lg" style="color: {getActivityColor(activity.type)}"></i>
            </div>
            ...
            <div class="flex-shrink-0">
              <i class="bi transition-transform" class:bi-chevron-down={expandedActivity !== activity.id} class:bi-chevron-up={expandedActivity === activity.id}></i>
            </div>
          </div>
            ...
            <div class="mt-4 pt-4 border-t border-gray-300 dark:border-gray-600 space-y-2">
              {#if activity.metadata.size}
                <div class="flex justify-between text-sm">
                  <span class="text-gray-600 dark:text-gray-400">{t('activity.size')}:</span>
              ...
              {/if}
              <div class="flex justify-between text-sm">
                <span class="text-gray-600 dark:text-gray-400">{t('activity.timestamp')}:</span>
                <span class="font-medium text-gray-900 dark:text-white">
              ...
              {#if activity.metadata.description}
                <div class="flex justify-between text-sm">
                  <span class="text-gray-600 dark:text-gray-400">{t('activity.description')}:</span>
                  <span class="font-medium text-gray-900 dark:text-white">
