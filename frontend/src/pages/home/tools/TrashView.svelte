<script>
  ...
  async function confirmBulkDelete() {
    ...
    try {
      ...
      await Promise.all(itemsToDelete.map(id => api.trash.permanentDelete(id)));
  ...
  async function deleteItemPermanently(itemId) {
    if (!confirm(t("trash.confirmDelete") || "Datei endgültig löschen?")) return;
  ...
  async function emptyTrash() {
    if (!confirm(t("trash.confirmEmptyTrash") || "Papierkorb vollständig leeren?")) return;
  ...
  <div class="max-w-7xl mx-auto">
      ...
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white mb-2">
            <i class="bi bi-trash text-red-600 dark:text-red-400 mr-3" aria-hidden="true"></i>
            {t("trash.title") || "Papierkorb"}
          </h1>
          <p class="text-sm text-gray-600 dark:text-gray-400">
            {t("trash.subtitle") || "Gelöschte Dateien verwalten und wiederherstellen"}
          </p>
        </div>
    ...
    >
      ...
      <div class="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between mb-6">
        ...
        <div class="flex-1 max-w-md">
          <div class="relative">
            <input
              type="text"
              placeholder={t("trash.searchPlaceholder") || "Dateien durchsuchen..."}
        ...
        <div class="flex items-center space-x-2">
          <select bind:value={sortBy} class="form-input">
            <option value="deleted_at">{t("trash.sortByDate") || "Nach Datum"}</option>
            <option value="file_name">{t("trash.sortByName") || "Nach Name"}</option>
        ...
        <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4 mb-6">
          <div class="flex items-center justify-between">
            <div class="flex items-center space-x-3">
              <i class="bi bi-check-circle text-blue-600 dark:text-blue-400" aria-hidden="true"></i>
              <span class="text-body">
                {selectedCount} {t("trash.itemsSelected") || "Dateien ausgewählt"}
              </span>
            </div>
      ...
      {#if filteredItems.length > 0}
        <div class="space-y-2">
          <!-- Header Row -->
          <div class="flex items-center p-3 bg-gray-50 dark:bg-slate-700/50 rounded-lg">
            <label class="flex items-center mr-4">
              ...
              />
              <span class="text-caption">{t("common.selectAll") || "Alle auswählen"}</span>
            </label>
            
            <div class="flex-1 grid grid-cols-3 gap-4">
              <span class="text-caption font-medium">{t("common.fileName") || "Dateiname"}</span>
              <span class="text-caption font-medium">{t("common.size") || "Größe"}</span>
              <span class="text-caption font-medium">{t("trash.deletedAt") || "Gelöscht am"}</span>
            </div>
                ...
                <div class="flex-1 grid grid-cols-3 gap-4 items-center">
                  ...
                  <div class="flex items-center space-x-3">
                    ...
                    <span class="text-body truncate" title={item.file_name}>
                      {item.file_name || t("common.unknownFile") || "Unbekannte Datei"}
                    </span>
                  </div>
      ...
      {:else if !loading}
        ...
        <div class="text-center py-12">
          <i class="bi bi-trash text-6xl text-gray-400 mb-4" aria-hidden="true"></i>
          ...
          <p class="text-caption">
            {searchQuery ?
              t("trash.tryDifferentSearch") || "Versuchen Sie eine andere Suche" :
              t("trash.noDeletedFiles") || "Keine gelöschten Dateien vorhanden"
            }
          </p>
        </div>
      {/if}
...
>
  <div class="space-y-4">
    <p class="text-body">
      {t("trash.restoreConfirmMessage") || "Möchten Sie die ausgewählten Dateien wiederherstellen?"}
    </p>
    <div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-3">
      <p class="text-caption">
        <strong>{selectedCount}</strong> {t("trash.itemsWillBeRestored") || "Dateien werden wiederhergestellt"}
      </p>
    </div>
  </div>
</UIModal>
    ...
    <div class="flex items-start space-x-3">
      ...
      <div>
        <p class="text-body font-medium mb-2">
          {t("trash.permanentDeleteWarning") || "Diese Aktion kann nicht rückgängig gemacht werden!"}
        </p>
        <p class="text-caption">
          {t("trash.permanentDeleteDescription") || "Die ausgewählten Dateien werden endgültig gelöscht und können nicht wiederhergestellt werden."}
        </p>
      </div>
    </div>
    <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-3">
      <p class="text-caption">
        <strong>{selectedCount}</strong> {t("trash.itemsWillBeDeleted") || "Dateien werden endgültig gelöscht"}
      </p>
    </div>
  </div>
</UIModal>