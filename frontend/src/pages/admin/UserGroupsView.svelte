<script>
  ...
  async function suspendUser() {
    ...
    error = '';
    try {
      await api.groups.suspendUser(suspendUserId, suspendReason, suspendDuration);
...
<div class="p-6 max-w-7xl mx-auto">
  ...
  <div class="flex items-center justify-between mb-6">
    ...
    <div class="flex gap-2">
      <button class="btn btn-ghost btn-sm" onclick={loadGroups} disabled={loading}>
        ...
        <div class="card bg-base-200 shadow-lg">
          <div class="card-body">
            ...
            <div class="flex items-start justify-between mb-4">
              <div>
                ...
                {#if selectedGroup.description}
                  <p class="text-sm text-base-content/60 mt-1">{selectedGroup.description}</p>
                {/if}
                <p class="text-xs text-base-content/40 mt-2">
                  {$t('groups.createdAt')}: {formatDate(selectedGroup.created_at)}
                </p>
              </div>
              <div class="flex gap-2">
                <button 
            ...
            {#if groupMembers.length === 0}
              <div class="text-center py-8 text-base-content/60 bg-base-100 rounded-lg">
                          ...
                          <div class="flex items-center gap-3">
                            <div class="avatar placeholder">
                              <div class="bg-neutral text-neutral-content rounded-full w-8">
                                <span class="text-sm">{(member.username || 'U').charAt(0).toUpperCase()}</span>
                              </div>
                            </div>
                        ...
                        <td class="text-right">
                          <div class="flex justify-end gap-1">
                            <button 
                              ...
                              onclick={() => openSuspendModal(member.user_id, member.username)}
                            >
  ...
  <div class="modal modal-open">
    <div class="modal-box">
      ...
      <div class="modal-action">
        <button class="btn btn-ghost" onclick={closeCreateModal} disabled={saving}>
          {$t('common.cancel')}
        </button>
        <button class="btn btn-primary" onclick={createGroup} disabled={saving || !newGroupName.trim()}>
          {#if saving}
    ...
    <div class="modal-box">
      ...
      <div class="py-4">
        <div class="form-control">
          ...
          <select class="select select-bordered w-full" bind:value={selectedUserId}>
            <option value="">{$t('groups.selectUserPlaceholder')}</option>
            {#each availableUsers as user}
              <option value={user.id}>{user.username} {user.email ? `(${user.email})` : ''}</option>
            {/each}
          </select>
        </div>
      </div>
      ...
      <div class="modal-action">
        <button class="btn btn-ghost" onclick={closeAddMemberModal} disabled={saving}>
          {$t('common.cancel')}
        </button>
        <button class="btn btn-primary" onclick={addMember} disabled={saving || !selectedUserId}>
          {#if saving}
            <span class="loading loading-spinner loading-sm"></span>
          {:else}
            <i class="bi bi-check-lg"></i>
...
{#if showDeleteModal}
  <div class="modal modal-open">
    <div class="modal-box">
      ...
      <div class="modal-action">
        <button class="btn btn-ghost" onclick={() => showDeleteModal = false} disabled={saving}>
          {$t('common.cancel')}
        </button>
...
{#if showSuspendModal}
  <div class="modal modal-open">
        ...
        <div class="form-control">
          ...
          <div class="flex gap-2">
            <input 
              ...
              max="365"
            />
            <span class="btn btn-ghost pointer-events-none">{$t('groups.days')}</span>
          </div>
        </div>
      </div>

      <div class="modal-action">
        <button class="btn btn-ghost" onclick={() => showSuspendModal = false} disabled={saving}>
          {$t('common.cancel')}
        </button>
        <button class="btn btn-warning" onclick={suspendUser} disabled={saving || !suspendReason.trim()}>
          {#if saving}
            <span class="loading loading-spinner loading-sm"></span>
          {:else}
            <i class="bi bi-pause-circle"></i>
    ...
    <div class="modal-backdrop" onclick={() => showSuspendModal = false}></div>
  </div>
{/if}
