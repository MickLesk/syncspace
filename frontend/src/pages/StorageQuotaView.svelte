  ...
  {:else if $storageQuota.error}
    ...
    >
      <div class="flex items-center">
        <i
          class="bi bi-exclamation-circle text-red-600 dark:text-red-400 mr-3"
          aria-hidden="true"
        ></i>
  ...
  {:else}
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
      ...
      >
        ...
        <p class="text-2xl font-bold text-green-900 dark:text-green-100">
          {formatStorageSize($storageQuota.totalUsed)}
        </p>
        <p class="text-xs text-green-600 dark:text-green-400 mt-1">
          {$storageQuota.percentUsed}% {t("storage.of_quota")}
        </p>
      </div>
      ...
      >
        ...
        <p class="text-xs text-green-600 dark:text-green-400 mt-1">
          {formatStorageSize($storageQuota.totalQuota)}
        </p>
      </div>
      ...
      >
        ...
        <div class="flex items-center">
          <i
            class="bi bi-circle-fill text-xl mr-2"
            class:text-green-500={$storageStatus === "success"}
            class:text-yellow-500={$storageStatus === "warning"}
            class:text-red-500={$storageStatus === "danger"}
            aria-hidden="true"
          ></i>
          <span
            class="text-lg font-semibold"
            class:text-green-700={$storageStatus === "success"}
            class:text-yellow-700={$storageStatus === "warning"}
            class:text-red-700={$storageStatus === "danger"}
            class:dark:text-green-100={$storageStatus === "success"}
            class:dark:text-yellow-100={$storageStatus === "warning"}
            class:dark:text-red-100={$storageStatus === "danger"}
          >
            {$storageStatus === "success"
              ? t("storage.status_good")
              : $storageStatus === "warning"
                ? t("storage.status_warning")
                : t("storage.status_critical")}
          </span>
        </div>
      </div>
    </div>
    ...
    <div class="mb-8">
      <div class="flex justify-between items-center mb-2">
        ...
        <p class="text-sm font-semibold {getStatusColor($storageStatus)}">
          ...
          {#each $typeBreakdown as item}
            ...
            >
              <div class="flex items-center justify-between mb-2">
                <div class="flex items-center">
                  <i
                    class="bi {item.icon} text-lg mr-3"
    ...
    <div class="mt-8 pt-8 border-t border-gray-200 dark:border-gray-700">
      ...
      >
        ...
        {t("storage.by_folder")}
        <i
          class="bi ml-2 transition-transform"
          class:bi-chevron-down={!showFolderBreakdown}
          class:bi-chevron-up={showFolderBreakdown}
        ></i>
      </button>
          ...
          {#each $storageQuota.byFolder as folder}
            ...
            >
              <div class="flex items-center justify-between mb-2">
                <div class="flex items-center">
                  <i
                    class="bi bi-folder-fill text-yellow-500 mr-3 text-lg"
                    aria-hidden="true"
                  ></i>
                  <div>
    ...
    {#if $storageQuota.lastUpdated}
      <p class="text-xs text-gray-500 dark:text-gray-400 mt-6 text-center">
        {t("storage.last_updated")}
        {new Date($storageQuota.lastUpdated).toLocaleString()}
      </p>
    {/if}
  {/if}
</div>
