<script>
  ...
  async function testPerformanceAPI() {
    ...
    try {
      ...
      const metrics = await performanceMonitor.loadMetrics();
      addResult(`✅ Metrics loaded: CPU ${metrics?.cpu_usage || 0}%`);
    } catch (error) {
    ...
    try {
      ...
      const cache = await performanceMonitor.loadCacheStats();
      addResult(
        `✅ Cache stats loaded: ${cache?.memory_cache_entries || 0} entries`
      );
    } catch (error) {
    ...
    try {
      ...
      const system = await performanceMonitor.loadSystemInfo();
      addResult(`✅ System info loaded: ${system?.cpu_cores || 0} cores`);
    } catch (error) {
    ...
    try {
      ...
      const jobs = await performanceMonitor.loadBackgroundJobs();
      addResult(
        `✅ Background jobs loaded: ${jobs?.queue_length || 0} in queue`
      );
    } catch (error) {
