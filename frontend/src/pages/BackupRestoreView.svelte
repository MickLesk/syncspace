...
<div class="p-6 max-w-7xl mx-auto">
  ...
  <div class="mb-8">
    <h1 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">
      <i
        class="bi bi-arrow-counterclockwise inline-block mr-3"
        aria-hidden="true"
      ></i>
      {$t("backup.title")}
    </h1>
  ...
  <!-- Controls -->
  <div
    class="bg-white dark:bg-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700 mb-8"
  >
  ...
  {#if $isRestoring}
    <div
      class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 rounded-lg p-4"
    >
      <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full">
        ...
        <div
          class="w-full bg-gray-200 dark:bg-gray-700 rounded-lg h-4 overflow-hidden mb-4"
        >
          <div
        ...
        {#if $restoreError}
          <div
            class="bg-red-50 dark:bg-red-900 border border-red-200 dark:border-red-700 rounded-lg p-3 mb-4 text-red-700 dark:text-red-200"
          >
  ...
  <div class="space-y-4">
    {#if $filteredBackups.length === 0}
      ...
      >
        <i
          class="bi bi-inbox text-4xl text-gray-400 mb-4 block"
          aria-hidden="true"
        ></i>
        <p class="text-gray-600 dark:text-gray-400">
          {$t("backup.no_backups")}
        ...
        >
          ...
          >
            <div class="flex items-center gap-4 flex-1">
              ...
              <div class="text-2xl">
                {#if backup.type === "full"}
                  <i class="bi bi-folder-fill text-green-500" aria-hidden="true"
                  ></i>
                {:else if backup.type === "incremental"}
                  <i
                    class="bi bi-arrow-up-circle-fill text-green-500"
                    aria-hidden="true"
                  ></i>
                {:else}
                  <i
                    class="bi bi-diagram-2-fill text-purple-500"
                    aria-hidden="true"
                  ></i>
                {/if}
              </div>
              ...
              <div class="flex-1">
                ...
                <div class="text-sm text-gray-600 dark:text-gray-400 mt-1">
                  {new Date(backup.date).toLocaleString("de-DE")} • {formatSize(
                    backup.size
              ...
              <div class="flex items-center gap-2">
                {#if backup.status === "success"}
                  <span
                    class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200"
                  >
                ...
                {:else if backup.status === "failed"}
                  <span
                    class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium bg-red-100 dark:bg-red-900 text-red-800 dark:text-red-200"
                  >
                ...
                {:else if backup.status === "in_progress"}
                  <span
                    class="inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm font-medium bg-yellow-100 dark:bg-yellow-900 text-yellow-800 dark:text-yellow-200"
                  >
            ...
            <!-- Expand Arrow -->
            <i
              class={`bi bi-chevron-down ml-4 transition ${expandedBackupId === backup.id ? "rotate-180" : ""}`}
            ></i>
          </button>
          ...
          {#if expandedBackupId === backup.id}
            ...
            >
              <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                <div>
                  <div
                    class="text-xs text-gray-600 dark:text-gray-400 uppercase font-semibold"
                  >
                    {$t("backup.duration")}
                  </div>
                  <div
                ...
                <div>
                  ...
                  <div
                    class="text-lg font-semibold text-gray-900 dark:text-white"
                  >
                    {(backup.compressionRatio * 100).toFixed(0)}%
                  </div>
                </div>
                <div>
                  <div
                  ...
                  <div class="text-lg font-semibold">
                    {#if backup.verificationStatus === "verified"}
                      <span class="text-green-600 dark:text-green-400"
                        >{$t("backup.verified")}</span
                      >
                    {:else if backup.verificationStatus === "verifying"}
                      <span class="text-yellow-600 dark:text-yellow-400"
                        >{$t("backup.verifying")}</span
                      >
                    {:else}
                      <span class="text-gray-600 dark:text-gray-400"
                        >{$t("backup.pending")}</span
                      >
                    {/if}
                  </div>
                </div>
              ...
              {#if backup.notes}
                <div
                  class="mb-6 p-3 bg-green-50 dark:bg-green-900 border border-green-200 dark:border-green-700 rounded-lg"
                >
              ...
              <div
                class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700 flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400"
              >
                <i
                  class={`bi ${backup.encryptionEnabled ? "bi-lock-fill text-green-600 dark:text-green-400" : "bi-unlock"}`}
                ></i>
                {backup.encryptionEnabled
                  ? $t("backup.encrypted")
                  : $t("backup.not_encrypted")}
              </div>
            </div>
          {/if}
        </div>
